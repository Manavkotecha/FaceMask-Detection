...
async def predict_video(
    ...
    max_frames: int = Query(default=300, ge=10, le=1000, description="Maximum frames to process (prevents long processing)"),
    background_tasks: BackgroundTasks = BackgroundTasks(),
):
    """
    Upload a video file and receive an annotated video with face-mask detections.

    The video is processed frame-by-frame with a maximum frame limit to prevent
    excessive processing time. Each processed frame gets bounding boxes with labels
    and confidence scores drawn on it.
    ...
    try:
        ...
        print(f"🎬 Processing video: {frames_to_process} frames max, {fps} FPS")

        while frame_idx < frames_to_process:
            ret, frame = cap.read()
            if not ret:
                print(f"⚠️  End of video reached at frame {frame_idx}")
                break

            # Validate frame
            if frame is None or frame.size == 0:
                print(f"⚠️  Invalid frame at index {frame_idx}, skipping")
                frame_idx += 1
                continue
            ...
            if frame_idx % frame_skip == 0:
                ...
                if frames_processed % 10 == 0:
                    print(f"📊 Processed {frames_processed} frames, detections: {sum(total_counts.values())}")
            else:
            ...
            # Timeout check (30 seconds max processing time)
            if time.time() - start_time > 30:
                print("⏰ Processing timeout reached (30s)")
                break
